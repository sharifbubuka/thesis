...
class CompactViltOutput(NamedTuple):
    """
    Complete output returned by CompactViltModel.

    Prediction outputs:
        logits:
            Raw answer logits with shape
            [batch_size, number_of_classes].

        scores:
            Independent sigmoid answer scores with shape
            [batch_size, number_of_classes].

        predicted_ids:
            Highest-scoring answer ID for each sample.

        confidence:
            Highest sigmoid score for each sample.
...
class MultimodalSequenceOutput(NamedTuple):
    """
    Output returned by the multimodal sequence builder.

    Shapes:
        embeddings:
            [batch_size, multimodal_sequence_length, hidden_size]

        attention_mask:
            [batch_size, multimodal_sequence_length]
    """
...
class FusionTransformerOutput(NamedTuple):
    """
    Output returned by the multimodal fusion Transformer.
    ...
    Shapes:
        ...
        hidden_states:
            One tensor for the input sequence and one for every
            Transformer layer.

        attentions:
            One attention tensor per Transformer layer with shape:
            [batch_size, heads, sequence_length, sequence_length]
    """
...
class ClassificationHeadOutput(NamedTuple):
    """
    Output returned by the soft-target VQA classification head.
    ...
    Shapes:
        ...
        scores:
            Independent sigmoid scores with shape:
            [batch_size, number_of_classes]
